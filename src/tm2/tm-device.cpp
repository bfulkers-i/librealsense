...
namespace librealsense
{
    ...
    void tm2_sensor::stream_endpoint()
    {
        ...
        while(!_stream_endpoint_thread_stop) {
            ...
            if (e == platform::RS2_USB_STATUS_TIMEOUT) {
                ...
                LOG_ERROR("Bulk libusb error " << platform::usb_status_to_string.at(e));
                continue;
            }
    ...
    void tm2_sensor::print_logs(const std::unique_ptr<bulk_message_response_get_and_clear_event_log> & log)
    {
        ...
        for (size_t i = 0; i < n_entries; i++) {
            ...
            LOG_WARNING("T265 FW message: " << timestamp << ": [0x" << e.threadID << "/" << e.moduleID << ":" << e.lineNumber << "] " << e.payload);
        ...
        device(ctx, group, register_device_notifications)
    {
        ...
        usb_info = group.usb_devices[0];
        usb_device = platform::usb_enumerator::create_usb_device(usb_info);
        auto interfaces = usb_device->get_interfaces();
        for(const auto & interface : interfaces) {
            auto endpoints = interface->get_endpoints();
            for(const auto & endpoint : endpoints) {
                ...
                if      (addr == ENDPOINT_HOST_IN)  endpoint_bulk_in = endpoint;
                    ...
                    LOG_ERROR("Unknown endpoint address " << addr);

                int ep_type = endpoint->get_type();
                int direction = endpoint->get_direction();
                int interface = endpoint->get_interface_number();
                LOG_ERROR("Endpoint addr: " << addr << " type: " << ep_type << " direction: " << direction << " interface: " << interface);
            }
        }
    ...
    platform::usb_status tm2_device::bulk_request_response(const Request &request, Response &response, size_t max_response_size, bool assert_success)
    {
        ...
        if (e == platform::RS2_USB_STATUS_NO_DEVICE) {
            ...
            LOG_ERROR("libusb_error " << platform::usb_status_to_string.at(e));
            return e;
        }
        ...
        transferred = 0;
        e = usb_messenger->bulk_transfer(endpoint_msg_in, (uint8_t*)&response, max_response_size, transferred, USB_TIMEOUT);
